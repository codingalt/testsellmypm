...
const addAdvisor = async(req,res)=>{
    const {name, title,location,dealSize,since,dealsClosed,expertise,shortBio,linkdin,website} = req.body;
            ...
            for(let i=0; i<profilePic.length; i++){
                ...
                console.log(companyLogoLink);
            // req.body.profilePicture = companyLogoLink;
...
const getAdvisors = async(req,res)=>{
    try {
        const search = new SearchListing(advisorModel.find({}), req.query).searchAdvisors();
...
const sortByExpertise = async(req,res)=>{
    ...
    try {
        ...
        // const advisors = await advisorModel.aggregate(
        //     [
        //         {
        //             $project: {
        //                 expertise: {
...
const sortByDealsClosed = async(req,res)=>{
    ...
    try {
        const advisors = await advisorModel.find({dealsClosed: {$gte: greaterThan, $lte: lessThan}});
        res.status(200).json({advisors,success: true});
    } catch (error) {
        res.status(500).json({message:error.message,success:false})
    }
...
module.exports = {addAdvisor, getAdvisors,getAdvisorById,getRelevantAdvisors,sortByExpertise,sortByDealsClosed}