  ...
  try {
    ...
    if (req.rootUser.others.subscription.length !== 0) {
      const lastSubscription =
        req.rootUser.others.subscription.slice(-1)[0]._id;
    ...
    const newSubscription = await userModel.updateOne(
      { _id: userId },
      { $push: { subscription: subscriptionDetails } },
      {
        new: true,
        useFindAndModify: false,
      }
    );
    const updateIsPaid = await userModel.updateOne(
      { _id: userId },
      { isPaid: true },
      {
        new: true,
