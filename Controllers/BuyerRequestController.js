...
const sendBuyerRequest = async (req, res) => {
  ...
  try {
    ...
    if(fromUserId === toUserId){
      res.status(400).json({message: "You cannot send request to your own listings", success: false})
      return;
    }
    // Check whether user is a paid/Member or not
    const user = await UserModel.findOne({_id: fromUserId});
    if(user.isPaid){
      const buyerRequest = new buyerRequestModel({ fromUserId, fromUserName: user.name, toUserId, listing });
      const createBuyerRequest = await buyerRequest.save();
      res.status(200).json({createBuyerRequest,message: "Buyer Request Sent Successfully.."});
    }else{
      res.status(401).json({message: "Your are not a Paid Member! Please Subscribe Any Package to Make Buyer Request",success: false})
    }
  } catch (error) {
