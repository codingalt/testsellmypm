...
} = require("../Models/sell/PropertyManagerServices");
...
const createListing = async (req, res) => {
  ...
  try {
    ...
    const createdDate =
      req.rootUser.others.subscription.slice(-1)[0].createdDate;
...
// Get All Listings from Admin side
...
const updateListing = async (req, res) => {
  ...
  try {
    const listings = await listingModel.findByIdAndUpdate(
      listingId,
      {
        "details.title": title,
        "details.summary": summary,
        "saleDetails.askingPrice": askingPrice,
        "saleDetails.specificPrice": specificPrice,
      },
      {
        new: true,
